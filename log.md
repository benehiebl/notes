...
## 2026-08-03 — Revised "Performance metrics on test datasets" in ESSD Landsat manuscript

**Updated**:
- `03_papers/03_landsat_data_publication/essd_landsat_manuscript/essd_landsat_manuscript.tex` — rewrote Sec. 3.1 from bullet placeholders into four prose subsections: LTC regression, DTS classification, pure vs. mixed stands, and comparison with published products/scope of metrics. Corrected the EVE/CON label mix-up in the original draft (12.5/9.7 %cover MAE are EVE and DEC, not EVE and CON). Added critical points: (i) EVE error is driven by a few large residuals (RMSE/MAE 1.53 vs 1.28 for DEC) plus the sub-canopy evergreen stratum that Landsat sees only weakly outside leaf-off [[chastain_2007_eve_landsat_understory]]; (ii) negative CON R² on VPO is a low-reference-variance artifact, not a model failure; (iii) best-match estimator is optimistically biased by construction and cannot be read as a corrected accuracy; (iv) CFI test labels are *not* independent (same source as rsCFI/csCFI training samples, pure-stand polygons by construction) → upper bound, not operational accuracy; (v) small per-class supports (n=15–30) forbid class-level accuracy claims; (vi) all metrics are plot-based on spatially clustered non-probability samples, not area-adjusted in the sense of Olofsson et al. 2014
- `03_papers/03_landsat_data_publication/essd_landsat_manuscript/essd_landsat_manuscript.bib` — added 4 references, all metadata verified via Crossref/publisher: `blickensdorfer_national_2024` (pure-stand F1 72–97%, −4–14 pp when mixed stands included), `bolyn_mapping_2022` (species-proportion R²_adj 0.50), `olofsson_good_2014` (area-adjusted accuracy good practice), `chastain_use_2007` (evergreen understory under deciduous canopy from leaf-off Landsat). Drew on existing wiki notes [[blickensdörfer_2024_tree_species]], [[bolyn_2022_tree_species_mapping]], [[miettinen_2025_forest_maps_europe]], [[grabska_2024_tree_species_map]], [[hemmerling_2021_forest_mapping_s2]]

**Open** (marked in-place with red TODO in the .tex): VDBI regression values (Tab. 3 row is still TODO), overall-accuracy cells in Tab. 2, stratified pure/mixed MAE-RMSE values, probabilistic/calibration metrics, DTS confusion matrix figure

## 2026-08-03 — Sourced the study-area elevation range in the ESSD Landsat manuscript

**Updated**:
- `03_papers/03_landsat_data_publication/essd_landsat_manuscript/essd_landsat_manuscript.tex` (Sec. 2.1 Study Area) — replaced the unsourced claim "altitudinal range from sea level to approximately 2500 m.s.l. \citep{needs_cite}" with two sourced figures: INFC third-survey forest area by 500 m altitude class (34.1% below 500 m a.s.l., 1.1% above 2000 m a.s.l.; Sec. 7.3, Table 7.11 of the open-access volume) [[gasparini_2022_nfi_italy]], and the western Italian Alps treeline at 2505 m a.s.l. (2000) / 2515 m a.s.l. (2008) [[leonelli_2011_treeline_italian_alps]]. This was the last `needs_cite` placeholder — the manuscript now compiles with zero undefined citations.
- `03_papers/03_landsat_data_publication/essd_landsat_manuscript/essd_landsat_manuscript.bib` — added `leonelli_climate_2011` (Leonelli, Pelfini, Morra di Cella & Garavaglia 2011, AMBIO 40(3):264–273, doi:10.1007/s13280-010-0096-2); metadata verified via Crossref, abstract and study region verified via BioOne

**Note**: INFC's top altitude class is open-ended (">2000 m") and therefore cannot support a 2500 m upper bound on its own — the treeline reference carries that part of the claim. Leonelli et al. report the treeline of three valleys in the *western* Italian Alps at a nearly undisturbed site, not a national maximum; the manuscript wording was made specific to match.
